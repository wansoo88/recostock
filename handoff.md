...
### A. Sentiment 수집기 — Week 1 step 1+2 **완료 (2026-05-17, GitHub Actions)**
- 소스 3개 (모두 무료, key 불필요):
...
- `sentiment/aggregator.py` — 일별 long-format `data/sentiment/sentiment_daily.parquet` 누적 upsert (publish lookback 2 days, dedup on (source, title, ticker))
...
- 첫 로컬 실행 (2026-05-17): SPY 5(yahoo 4 + edgar 1), QQQ 3, XLF/XLK 2, DIA/XLE/XLP 1
- **다음 step**: Reddit (사용자 API key 발급 후) → FinBERT 점수화 → v4 모델 통합 (3주 일정 유지)
...
2. ~~**다음 주** — A Phase Week 1 (Reddit + RSS, 무료 API)~~ **2026-05-17 step 1+2 완료 (Yahoo RSS + HN + EDGAR). Reddit은 사용자 key 발급 후 추가**
