...
### A. Sentiment 수집기 — Week 1 step 1+2+3 **완료 (2026-05-17, GitHub Actions)**
- 소스 3개 (모두 무료, key 불필요):
    - `sentiment/sources/yahoo_rss.py` — Yahoo Finance per-ticker RSS (18 ETF)
    - `sentiment/sources/hackernews.py` — HN Algolia search
    - `sentiment/sources/edgar.py` — SEC EDGAR full-text search (forms=8-K,10-Q,10-K,N-PORT,N-CSR,485BPOS); 각 ETF 정식 명칭(예: "Energy Select Sector SPDR") query로 filing 검색 → `$TICKER` cashtag 본문 주입해 ticker_extract가 매칭
- `sentiment/ticker_extract.py` — cashtag + bare ticker + ETF 이름 alias. 18 ETF (VXX 추가).
- `sentiment/scorer.py` — **FinBERT (ProsusAI/finbert) polarity ∈ [-1,+1] = P(pos)-P(neg)**. lazy import + transformers/torch 미설치 시 None (로컬 안전).
- `sentiment/aggregator.py` — long-format parquet 누적 upsert. 스키마: `(date, ticker, source, mention_count, polarity_mean, polarity_n)`. publish lookback 2 days, dedup on (source, title, ticker). 기존 4-col parquet은 자동 schema-evolve.
- `scripts/collect_sentiment.py` — entrypoint, `--dry-run` / `--no-score` 지원. 소스 하나 실패해도 진행.
- `.github/workflows/sentiment.yml` — daily 13:00 UTC, Mon–Fri. CPU torch + transformers 설치, `~/.cache/huggingface` 캐싱 (모델 weights 첫 실행만 다운로드).
- 첫 로컬 실행 (2026-05-17, no-score 모드): SPY 5(yahoo 4 + edgar 1), QQQ 3, XLF/XLK 2, DIA/XLE/XLP 1. polarity_n=0 (FinBERT는 workflow에서 첫 검증).
- **다음 step**: Reddit (사용자 API key 발급 후) → IC 검증 (~2주 누적 후) → v4 모델 통합
...
2. ~~**다음 주** — A Phase Week 1 (Reddit + RSS, 무료 API)~~ **2026-05-17 step 1+2+3 완료 (Yahoo RSS + HN + EDGAR + FinBERT polarity). Reddit은 사용자 key 발급 후 추가**
3. **2-3주 후** — IC 검증 + v4 모델 학습 (sentiment 피처 통합)
